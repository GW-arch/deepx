...
def build_kit(sample_rate: int = 22050) -> dict[str, pygame.mixer.Sound]:
    pygame.mixer.pre_init(sample_rate, size=-16, channels=2, buffer=128)
    pygame.mixer.init()
...
# Left hand descends from thumb to pinky so the thumb is highest and pinky is lowest.
...
def piano_slots_from_inter_hand_distance(
    ...
    *,
    ...
    left = tuple(reversed(pentatonic_five(root_l)))
...
def _piano_tone(sample_rate: int, freq_hz: float, duration: float = 0.50) -> np.ndarray:
    n = int(sample_rate * duration)
    t = np.arange(n, dtype=np.float64) / sample_rate
    env = np.exp(-t * 4.0) * (1.0 - np.exp(-t * 180.0))  # short attack, longer tail
