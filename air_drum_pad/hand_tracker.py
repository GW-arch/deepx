...
class DxnnHandTracker:
    ...
    def _build_input(self, rgb_region: np.ndarray) -> np.ndarray:
        ...
        tensor_layout = str(inp_cfg.get("tensor_layout", "auto"))
        if tensor_layout == "auto":
            layout = self._layout_hw
        elif tensor_layout in ("nhwc", "nchw"):
            layout = tensor_layout
        else:
            raise ValueError(tensor_layout)
    ...
    def _infer_region(
        ...
        *,
        ...
        return self._postprocess(outs, x_scale=x_scale, x_bias=x_bias)
    ...
    def _postprocess(
        ...
        *,
        ...
        flat = np.asarray(outs[li]).astype(np.float32).reshape(-1)
